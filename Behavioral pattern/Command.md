...
**<div align="center">이 램프를 켜는 기능을하는 버튼을 설계하면 아래와 같다.</div>**
...
지금까지 살펴보았듯 Stratrgy 패턴과 Command 패턴은, "변화되는 것을 식별하고 식별된 것을 클래스로 모델링하고,
이 클래스들을 포괄하는 상위개념을 만들고 그 상위개념을 사용하게끔 설계하는 것" 이라는 공통점이있다.  

Command 패턴과 Strategy 패턴은 큰 차이가 없어보인다. 그렇다면 이 둘의 차이점은 대체 뭘까?

<br>

### 명령-기능을 수행하는 주체의 차이 

 이 Command 패턴의 예제에서, 실제 램프를 켜는건 Lamp 객체, 알람을 시작하는건 Alarm객체로 

 명령을 수행하는 주체가 각각 다르다. 

 반면 Strategy 패턴의 펭수 예제의 경우, 걷거나 점프하는 것의 주체는 펭수로 동일한 주체이다.

<br>

### 명령-기능의 목적이 동일한가 아닌가
또한, walk 나 jump는 "펭수를 움직이게한다"는 동일한 개념에 속한다. 

즉, walk 나 jump처럼 각 기능이 동일한 개념의 목적(move)을 수행하는 경우에는 Strategy 설계패턴을 적용하면 된다.


그러나, 우리가 보고있는 Command 예제에서는 "램프가 켜진다", "알람이 시작한다"와 같은 명령 및 기능들은 서로 동일하지 않은 목적을 가지고있지 않다. 램프가 켜지는거랑 알람이 시작되는거랑 대체 무슨 연관인지, 어떻게 연관지어야할지 도저히 생각나지 않는다.   

<br>

즉, **`동일한 목적으로 같이 둘 수 없는 목적이 동일하지않은 요청-명령들이고` + `이 명령을 수행하는 주체가 다른경우에는` => `Command 패턴을 적용한다.`** 
