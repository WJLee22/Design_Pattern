...
### 명령-기능을 수행하는 주체의 차이 
 ...
 이 Command 패턴의 예제에서, 실제 노래를 부르는건 bird 객체, 티비를 켜는건 tv객체로 
...
또한, walk 나 jump는 움직인다는 동일한 개념에 속한다. 

즉, walk 나 jump처럼 각 기능이 동일한 개념의 목적(move)을 수행하는 경우에는 Strategy 설계패턴을 적용하면 된다.



그러나, 우리가 보고있는 Command 예제에서는 "새가 노래한다", "tv 전원이 켜진다"와 같은 명령 및 기능들은 서로 동일하지 않은 목적을 가지고있지 않다. 새가 노래하는거랑 Tv 전원 켜는거랑 대체 무슨 연관인지, 어떻게 연관지어야할지 도저히 생각나지 않는다.   
